...
class NestedCVOptimizer:
    ...
    def apply_imbalance_handling(self, X_train, y_train, technique):
        ...
        if technique == 'none':
            ...
            return SMOTE(random_state=self.random_state).fit_resample(X_train, y_train)
        elif technique == 'smote_tomek':
            return SMOTETomek(random_state=self.random_state).fit_resample(X_train, y_train)
    ...
    def inner_cv_objective(self, trial, X_train_outer, y_train_outer, groups_train_outer):
        ...
        for train_idx, val_idx in self.inner_cv.split(X_train_outer, y_train_outer, groups_train_outer):
            ...
            X_train_balanced, y_train_balanced = self.apply_imbalance_handling( # type: ignore
                X_train_inner, y_train_inner, params['imbalance_technique']
            )
