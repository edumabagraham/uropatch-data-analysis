...
class HPTuner:
    def __init__(self, X_train, y_train, groups_train, folds=5, 
                n_trials=100, random_state=42):
        """
        Hyperparameter tuner for binary classification
        """
        ...
        self.sgkf = StratifiedGroupKFold(n_splits=5, random_state=42)
    ...
    def tune_hyperparams(self, trial, X_train, y_train, groups_train):
        """Inner CV loop for hyperparameter tuning."""
        params = self.define_search_space(trial)
        fold_scores = []

        for train_idx, test_idx in self.sgkf.split(X_train, y_train, groups_train):
    ...
    def run_nested_cv(self):
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(self.sgkf.split(self.X_train, self.y_encoded, self.groups_train)):
